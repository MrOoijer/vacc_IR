...
pretty_date(df, main="Relative risk2 for fully without booster \nvaccinated adults in Israel")
...
tmp<- risk[, c(1:5, 9, 11)]
tmp$IRR <- with(tmp, f3/f1)
pretty_date(tmp[, c(1, ncol(tmp))], ylim=c(0,25), main="Inversed RR goes from 12 to 1 in 2 weeks??")
...
pretty_date(df, main="Results of lm on log(1/RR)", lwd=2)
df0<- df
df0$cases<- df$cases-l$residuals
...
Does not look right. The R^2 is mainly for the decline of the 
curve, and that's why we started looking at this regression in the first place, See the residuals.
...
With the above overall data, it does not make much sense to look at the behaviour of smaller (age) groups. We will probably see enother example of the ecological fallacy. 


Finally, in no way we see any added value to introduce a measure of VE=(1-RR). It is not found in the literature of epidemiology AFAIK, see f.i. https://www.wikilectures.eu/w/Attributable_and_Relative_Risks,_Odds_Ratio /
